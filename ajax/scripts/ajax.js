...
/**
 * Limpia texto y la información del los campos
 */
...
function procesarFetch(numsecs, user) {
  ...
  fetch(`${BASE_URL}${user}?delay=${numsecs}`)
    .then((response) => {
      //Si el estado es OK se actualiza el campo estado y devuelve
      //una promesa con los datos en json
      if (response.ok) {
    ...
    .then((data) => {
      ...
      //REALIZO LA SEGUNDA LLAMADA QUE HARÁ UN POST DEL USUARIO
      fetch(POSTMAN_URL, {
